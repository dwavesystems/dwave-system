...
schema = \
    ...
    CREATE TABLE IF NOT EXISTS system(
        name TEXT UNIQUE,  -- the name of the system
        id INTEGER PRIMARY KEY);

    CREATE TABLE IF NOT EXISTS flux_bias(
        chain_id INTEGER NOT NULL,
        system_id INTEGER NOT NULL,
        insert_time TEXT DEFAULT (datetime('now')),  -- flux biases are only valid for relatively short time
        flux_bias REAL NOT NULL,
        FOREIGN KEY (chain_id) REFERENCES chain(id),
        FOREIGN KEY (system_id) REFERENCES system(id),
        PRIMARY KEY(chain_id, system_id));
