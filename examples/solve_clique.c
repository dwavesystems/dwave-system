...
/* I make this with
gcc solve_clique.c -o solve_clique libdwave_sapi.so -lm
*/
...
int main() {
    ...
    */
    ...
    FILE *embedding_file = fopen("embedded_clique_largest","r");
    int qubit = -1;
    int n = 0;
    while(!feof(embedding_file) && fscanf(embedding_file, "%d", &qubit) && qubit != -1) {
        embedding.elements[qubit] = n;
    ...
    int *new_solutions = (int *)malloc(samples->num_solutions * n * sizeof(int));
